...
[After installing the Unit Testing extension](https://code.ileditor.dev/download), Code for IBM i will automatically detect test files. You will see ability to run tests right from the active editors, as well as see inline results in the active editor or in the Unit Test Explorer view.

Nothing needs to be installed on the host IBM i to make use of the Unit Testing framework.

## Example test walkthroughs

Unit Tests are RPGLE procedures that reside in a source member or streamfile. The name of the source must end with `.test` or `_t` (case-insensitive). For example, some valid names might be:
...
In most cases, you will want to test other service programs. You are able to use `CTL-OPT` to include a binding directory and also use the `/COPY` or `/INCLUDE` directives to include the prototypes.
...
Tests will run using the library list setup in your connection. They are built every time you run your tests/test. You will see the results update in the active editor and also update in the Explorer view.
